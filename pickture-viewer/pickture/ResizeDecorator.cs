    ...
    public class ResizeDecorator : Control
    {
        ...
        private void ShowAdorner()
        {
            if (this.adorner == null)
            {
                ...
                {
                    adornerLayer.ClipToBounds = true;

                    var ctrl = this.DataContext as Control;
                    ContentPresenter designerItem = ctrl.Template.FindName("PART_ContentPresenter", ctrl) as ContentPresenter;
                    
                    var host = Find.Parent<IEditorHost>(designerItem);
                    var canvas = host.Editor.Canvas;
